...
export const REMOVE_ADS_SKU = 'buildcalc_remove_ads';
const STORAGE_KEY = 'buildcalc.premium.v1';
...
export function PremiumProvider({ children }) {
  ...
  // Load the cached flag immediately so the UI doesn't flash ads for an owner.
  useEffect(() => {
    (async () => {
      try {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Clear dangling Android transactions left by a previous session.
        ...
        // Authoritative ownership check (handles reinstalls and new devices).
        try {
